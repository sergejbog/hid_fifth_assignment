<script lang="ts">
	...
	import { userProfile, userProfileStore } from '$lib/dummyData';
	import { FacebookSolid, XSolid, InstagramSolid } from 'flowbite-svelte-icons';
...
<div class="container mx-auto w-full px-4">
	<div class="flex justify-between space-x-12">
		...
		<Card size="md">
			<div class="mb-4 flex items-center">
				<img src={profile.avatar} alt={profile.name} class="mr-4 h-20 w-20 rounded-full" />
				<div>
					<h2 class="text-2xl font-bold">{profile.name}</h2>
					<p class="text-gray-600">Age: {profile.age}</p>
				</div>
			</div>
			<h3 class="mb-2 text-xl font-semibold">Platform Settings</h3>
			<div class="mb-4">
				<h4 class="mb-2 font-semibold">ACCOUNT</h4>
				<Toggle bind:checked={profile.settings.emailLikes} class="mb-2"
					>Email me when someone likes my playlist</Toggle
				>
				<Toggle bind:checked={profile.settings.emailReplies} class="mb-2"
					>Email me when someone replies to my post</Toggle
				>
				<Toggle bind:checked={profile.settings.emailMentions} class="mb-2"
					>Email me when someone mentions me in a post</Toggle
				>
			</div>
			<div>
				<h4 class="mb-2 font-semibold">APPLICATION</h4>
				<Toggle bind:checked={profile.settings.newLaunches} class="mb-2"
					>New launches and projects</Toggle
				>
				<Toggle bind:checked={profile.settings.monthlyUpdates} class="mb-2"
					>Monthly product updates</Toggle
				>
				<Toggle bind:checked={profile.settings.newsletter} class="mb-2"
		...
		<Card size="md">
			...
			<div class="space-y-4">
				{#each profile.playlists as playlist}
	...
	<div class="mb-4 grid w-full grid-cols-3 gap-4">
		{#each $userProfileStore.posts as post}
			<Card size="xl">
				<div class="mb-4 flex items-center">
					<img src={profile.avatar} alt={profile.name} class="mr-2 h-10 w-10 rounded-full" />
					<div>
